...
{
    [Authorize]
    [RoutePrefix("api/cases/chatbot")]
    public class CreateCaseThroughChatbotController : ApiController
    {
        [HttpPost]
        [Route("create")]
        public IHttpActionResult CreateCase(CreateCaseRequest request)
        {
            try
            {
                ...
                else if (!string.IsNullOrWhiteSpace(request.CompanyName))
                {
                    ...
                    incident["customerid"] =
                        new EntityReference("account", result.Entities.First().Id);
                }
                         ...
                         !string.IsNullOrWhiteSpace(request.LastName))
                {
                    ...
                    incident["customerid"] =
                        new EntityReference("contact", result.Entities.First().Id);
                }
                else
                {
                ...
                // ======================================
                // FORCE FORM TYPE = CHATBOT (VALUE = 3)
                // ======================================

                var updateEntity = new Entity("incident");
                updateEntity.Id = caseId;
                updateEntity["new_formtype"] = new OptionSetValue(3);
                ...
                return Ok(new
                {
                    ...
                    Status = createdCase.FormattedValues.Contains("statuscode")
                                ...
                                : null,
                    Message = "Chatbot case created successfully"
                });
            }
            catch (Exception ex)
            {
        ...
        // ======================================
        // NORMALIZER (kept for future use)
        // ======================================
